...
QGC_LOGGING_CATEGORY(LogDownloadControllerLog, "test.analyzeview.logdownloadcontroller")
...
{
    const QString dir = path.isEmpty() ? qgcApp()->toolbox()->settingsManager()->appSettings()->logSavePath() : path;
...
bool LogDownloadController::_prepareLogDownload()
{
    ...
    const QString ftime = (entry->time().date().year() < 2010) ? QStringLiteral("UnknownDate") : entry->time().toString(QStringLiteral("yyyy-M-d-hh-mm-ss"));
    ...
    if (_downloadData->file.exists()) {
        ...
        do {
            numDups +=1;
            _downloadData->file.setFileName(filename_spl[0] + '_' + QString::number(numDups) + '.' + filename_spl[1]);
